...
- **Wallet storage is NOT namespaced by network.** IndexedDB keys (`channel_manager`, channel monitors, `ldk_seed`, `network_graph`, `scorer`, …) are shared across networks. Switching the *same* browser profile between regtest and mainnet **overwrites/corrupts channel state** (symptom: `Loaded 0 channel monitors` + a manager from the wrong chain). Use one network per profile; recover via `importState()` from a backup. Namespacing storage keys by network is an open fix.

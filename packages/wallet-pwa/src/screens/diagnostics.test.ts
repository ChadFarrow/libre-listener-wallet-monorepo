...
describe("diagnostics card", () => {
  ...
  it("export in the native wrapper routes through the share-sheet plugin, not a blob download", async () => {
    console.log("[DiagTest] native marker");
    await diagFlushNow();

    const shareText = vi.fn().mockResolvedValue(undefined);
    const createObjectURL = vi.fn(() => "blob:diag");
    vi.stubGlobal("Capacitor", { isNativePlatform: () => true, Plugins: { LibreDiagnostics: { shareText } } });
    ...
    document.getElementById("diag-export")!.dispatchEvent(new Event("click"));
    await vi.waitFor(() => expect(shareText).toHaveBeenCalledTimes(1));

    // The native plugin got the log text and the browser blob path was NOT used.
    expect(shareText.mock.calls[0][0].contents).toContain("[DiagTest] native marker");
    expect(shareText.mock.calls[0][0].name).toMatch(/^libre-diag-mainnet-\d{4}-\d{2}-\d{2}-\d{4}\.txt$/);

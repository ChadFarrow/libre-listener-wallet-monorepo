...
### 5. Streaming-sats soak — sustained rate + latency
The V4V *streaming* workload: keep ONE wallet up and fire keysends on a timer (streaming splits, up
to ~20/min, plus an occasional boost fanning out into sequential splits). Unlike the restart soaks,
this measures per-payment **latency** — the thing that actually kills streaming: if a keysend takes
longer to settle than the streaming interval, payments back up and fall behind. Every failure (# +
error) and the latency min/avg/max are reported. It found (and pins the fix for) NWC keysend
settlement taking **15–30 s** — because a durable monitor update (Layer C) un-pauses the channel
asynchronously and nothing pumped `PeerManager.process_events()` after it, so each monitor-gated
commitment-dance step waited for the 10 s peer tick. Fix: pump `process_events()` in the
durable-persist ack (`index.ts`) → settlement dropped to **~1 s** (verified: 40 payments @ 20/min +
boosts of 6 splits, 0 failures, 943 ms avg).

```bash
# realistic: 10/min for 30 payments
...
# realistic V4V: 20/min stream + a 6-split boost every 15 payments
STREAM_PAYMENTS=40 STREAM_INTERVAL_MS=3000 STREAM_BOOST_EVERY=15 STREAM_BOOST_SPLITS=6 \
  pnpm --filter @libre/listener-wallet exec vitest run src/tests/integration/soak-nwc-streaming.test.ts
# stress: fire as fast as it settles / add mid-stream reloads (STREAM_RESTART_EVERY=N)
STREAM_PAYMENTS=100 STREAM_INTERVAL_MS=500 pnpm --filter @libre/listener-wallet exec vitest run ...
```

A `latency ms min/avg/max` well above the streaming interval (e.g. seconds when you want ≤1 s) is the
regression to watch — payments succeed but the effective throughput can't keep up with the rate.
